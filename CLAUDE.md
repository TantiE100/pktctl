...
- The IPC index (`crates/pktctl/assets/ipc-index.json`) is generated by
  `tools/ipc-index/generate.py`; never edit it by hand, and never commit one
  built with `--with-summaries`, which copies in Cisco's Javadoc prose. New
  typed features read signatures from it (`packet_tracer::api`) or from
  `describe_ipc`.

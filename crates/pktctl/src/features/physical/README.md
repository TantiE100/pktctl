...
- **Renaming**: there is no call to rename a location. New ones are named
  `City` and `Wiring Closet`, so creating two leaves duplicates. Packet Tracer
  only moves things into the first of two same-named siblings, so a move into
  `City#2` is refused with an explanation instead of landing in the wrong place.
